...
ramUsage = 7       # GB per job, adjust as needed

max_parallel_jobs = 90
sleep_interval = 20
KILL_TIMEOUT = 100000
...
def submit_job(channel, sh_file, job_map):
    ...
    # Submit new job
    try:
        cmd = f"qsub -q cupid -o localhost:/data/users/rserino/CROSS/RUN000136/output/ -m abe -M roberto.serino@ijclab.in2p3.fr -l walltime=24:00:00 -l mem={ramUsage}G {sh_file}"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    except Exception as e:
...
def try_submit_pass(pending_list, make_lines_fn, job_map, append_jobids_list=None, sleep_on_slot=False):
...
def get_random_timestamp(base_path, run_number, min_size_mb=100):
    ...
    if not valid_files:
        raise RuntimeError("No .bin files larger than 100 MB found")
    ...
    if not match:
        raise RuntimeError(f"Could not extract timestamp from {chosen_file}")
