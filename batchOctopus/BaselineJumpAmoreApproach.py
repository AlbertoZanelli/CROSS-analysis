...
---------------------------------------------------------------------------
MODE:
  "mergedrun" -> file mergiati, canale letto dal nome ('..._ch25_...'),
                 vengono processati i file che finiscono in '82.root'.
  "run"       -> file di singola run, nome tipo '25_73_74_000092.root'
                 (il PRIMO numero e' il canale); vengono processati i file
                 che matchano il pattern '<ch>_<..>_<..>_<runnumber>.root'.
"""
...
# ===========================================================================
# MODE  <- scegli la modalita'
# ===========================================================================

MODE = "mergedrun"        # "mergedrun" oppure "run"

# ===========================================================================
# PARAMETERS  <- tune these
# ===========================================================================

# --- percorsi modalita' "mergedrun" ---------------------------------------
MERGED_DIRECTORY    = "/data/users/azanelli/octopus_work/CROSS/MergedRuns/"
MERGED_DIRECTORYOUT = "/data/users/azanelli/octopus_work/CROSS/MergedRuns/CorrectedAmp/"
MERGED_DEBUG_DIR    = "/data/users/azanelli/octopus_work/CROSS/MergedRuns/CorrectedAmp/PlotFit/"

# --- percorsi modalita' "run" ---------------------------------------------
RUN_DIRECTORY       = "/data/users/azanelli/octopus_work/CROSS/RUN000092/Coincidence"
RUN_DIRECTORYOUT    = "/data/users/azanelli/octopus_work/CROSS/RUN000092/CorrectedAmp/"
RUN_DEBUG_DIR       = "/data/users/azanelli/octopus_work/CROSS/RUN000092/CorrectedAmp/PlotFit/"

# --- percorsi attivi (selezionati in base a MODE) -------------------------
if MODE == "run":
    DIRECTORY    = RUN_DIRECTORY
    DIRECTORYOUT = RUN_DIRECTORYOUT
    DEBUG_DIR    = RUN_DEBUG_DIR
elif MODE == "mergedrun":
    DIRECTORY    = MERGED_DIRECTORY
    DIRECTORYOUT = MERGED_DIRECTORYOUT
    DEBUG_DIR    = MERGED_DEBUG_DIR
else:
    raise ValueError(f"MODE sconosciuta: '{MODE}' (usa 'run' o 'mergedrun')")
...
CHANNELS_TO_PROCESS = [86]    # se NON vuota: processa SOLO questi canali (es. [25, 73])
...
def get_channel_from_filename(filepath):
    """Estrae il numero di canale dal nome file, in base a MODE.

    MODE == 'mergedrun' : '..._ch25_...'          -> 25
    MODE == 'run'       : '25_73_74_000092.root'  -> 25  (primo numero)
...
def find_root_files(directory):
    """Restituisce la lista (ordinata) dei file .root da processare, in base
    a MODE.

    MODE == 'mergedrun' : file con nome tipo '000097_000128_ch25_73_74.root'
                          (escludendo '_corr.root').
    MODE == 'run'       : file con nome tipo '25_73_74_000092.root'
                          cioe' <ch>[_<n>...]_<runnumber a 6 cifre>.root
                          (escludendo '_corr.root').
    """
    if MODE == "run":
        # <ch>_<..>_<..>_<runnumber(>=6 cifre)>.root
        pattern = re.compile(r"^\d+(?:_\d+)+\.root$")
        files = [
            os.path.join(directory, f)
            for f in os.listdir(directory)
            if pattern.match(f) and not f.endswith("_corr.root")
        ]
    else:
        # <run>_<run>_ch<ch>_<ch>_<ch>.root
        # Esempio: 000097_000128_ch25_73_74.root
        pattern = re.compile(r"^\d+(?:_\d+)*_ch\d+(?:_\d+)*\.root$")
        files = [
            os.path.join(directory, f)
            for f in os.listdir(directory)
            if pattern.match(f) and not f.endswith("_corr.root")
...
def load_data(filepath):
    ...
    for i in range(n):
        ...
        if MODE == 'mergedrun':
            ...
            ts   = float(t_ts.heat_timefromstartrun)
...
def write_corrected_tree(filepath, lookup, issignal_branch):
    ...
    if MODE == 'mergedrun':
        ...
        corr_tree.Branch("heat_timefromstartrun", ts_val,       "heat_timefromstartrun/D")
    ...
    for i in range(n_sig):
        ...
        if MODE == 'mergedrun':
            ...
            ts   = float(t_ts_out.heat_timefromstartrun)
...
def main():
    ...
    dirs = [DIRECTORY, DIRECTORYOUT, DEBUG_DIR]
    ...
    if not root_files:
        print(f"[ERR] No .root files found in: {directory}")
        sys.exit(1)
