...
LY_MIN_LD1, LY_MAX_LD1 = 0.000, 0.005
LY_MIN_LD2, LY_MAX_LD2 = 0.00005, 0.005
...
def copy_input_metadata(flist, outfile, runs):
    """
    Creates merge_metadata/RUN000XXX/global_metadata for each merged run.
    """
    outfile.cd()
    merge_dir = outfile.mkdir("merge_metadata")

    for fpath, run in zip(flist, runs):
        run_num = (int(run.replace("RUN", "")) if run.startswith("RUN") else int(run))
        run_label = f"RUN{run_num:06d}"

        f = ROOT.TFile.Open(fpath)
        if not f or f.IsZombie():
            print(f"Warning: could not open {fpath} for metadata copy")
            continue

        merge_dir.cd()
        run_dir = merge_dir.mkdir(run_label)

        # Try all three detectors' global_metadata — copy whichever exist
        for src_name in ("metadata_heat", "metadata_LD1", "metadata_LD2"):
            global_meta = f.Get(f"{src_name}/global_metadata")
            if not global_meta:
                continue

            run_dir.cd()
            gm_dir = run_dir.mkdir(f"{src_name}_global_metadata")
            gm_dir.cd()

            for key in global_meta.GetListOfKeys():
                obj = key.ReadObj()
                if obj:
                    obj.Write()
                    del obj
...
def merge_runs(cross_dir, runs, channel=None, channelLD1=None, channelLD2=None,
    ...
    for ch, flist in sorted(files_by_channel.items()):
        ...
        copy_input_metadata(flist, outfile, runs)
